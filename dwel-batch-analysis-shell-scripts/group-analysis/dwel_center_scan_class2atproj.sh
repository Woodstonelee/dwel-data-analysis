...
OUTFILENAME=${BASENAME:0:${#BASENAME}-4}"_atp2.img"

python $PYCMD -i $INFILENAME -o $OUTFILENAME -p 24 --pixelfunc "mode" -C -M --pulseno=8 --intensity=4 -H ${INSHEIGHTS[$SGE_TASK_ID-1]} --maxzen=117 -r 2.5 -N ${NORTHAZ[$SGE_TASK_ID-1]}