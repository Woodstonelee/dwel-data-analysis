...
def closest_points(ind1, ind2, value1, value2, thresh):
    ...
    # Check each side of the closes point pair to find any more point pairs.
    ...
    if n1 and n2:
        ...
        for i1 in range(n1-1, -1, -1):
            if i2 < 0:
                break
            x = value1[flag1][i1]
            tmpvdiff = np.fabs(value2[flag2][0:i2+1] - x)
            tmpind = np.argmin(tmpvdiff)
            if tmpvdiff[tmpind] < new_thresh:
                leftind1.appendleft(ind1[flag1][i1])
                leftind2.appendleft(ind2[flag2][0:i2+1][tmpind])
                i2 -= 1
    # if n1 and n2:
    #     i2 = n2 - 1
    #     i1 = n1 - 1
    ...
    return np.array(list(leftind1)+rightind1).astype(int), np.array(list(leftind2)+rightind2).astype(int)
...
def generate_spectral_points(nirpoints, swirpoints, rdiff_thresh=0.96, nr=3142, nc=1022):
    ...
    nirpoints[tmpind, 0] = -2*rdiff_thresh
    tmpind = np.where(swirpoints[:, 0] < 1e-10)[0]
    swirpoints[tmpind, 0] = -2*rdiff_thresh
    ...
    nir_shotind = (nirpoints[:, 1]-1)*nr + nirpoints[:, 2]
    swir_shotind = (swirpoints[:, 1]-1)*nr + swirpoints[:, 2]
    ...
    local_closest_points = closest_points2
    local_len = len
...
#    swirpoints = np.loadtxt(swirfile, dtype=np.float32, usecols=ind, delimiter=',', skiprows=headerlines)
    ...
    nir_ind, swir_ind, return_type, nu_shotnum, num_of_returns, return_num \
    ...
    headerstr = prefixstr + "x,y,z,d_I_nir,d_I_swir,return_number,number_of_returns,shot_number,range,theta,phi,sample,line,fwhm_nir,fwhm_swir,multi-return"
    # write to file
